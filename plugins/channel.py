...
async def send_movie_update(bot, base_name):
    ...
    for channel_id in MOVIE_UPDATE_CHANNEL:
        if str(channel_id) in message_ids:
            continue
        ...
        try:
            ...
            # Retry once for FloodWait if needed, but for simplicity we'll just catch and log
            try:
                 # Minimal retry logic
                 if is_photo and resized_poster:
                    msg = await bot.send_photo(chat_id=channel_id, photo=resized_poster, caption=text, reply_markup=buttons, parse_mode=enums.ParseMode.HTML)
                 else:
                    msg = await bot.send_message(chat_id=channel_id, text=text, reply_markup=buttons, parse_mode=enums.ParseMode.HTML)
                 message_ids[str(channel_id)] = msg.id
            except Exception:
                logger.error(f"Failed to send movie update to {channel_id} after FloodWait")
        except Exception as e:
            logger.error(f"Failed to send movie update to {channel_id}: {e}")
...
async def update_movie_message(bot, base_name):
    try:
        ...
        legacy_msg_id = movie_doc.get("message_id")
        if legacy_msg_id and not message_ids:
            # Map legacy message to the first configured channel
            if MOVIE_UPDATE_CHANNEL:
                message_ids[str(MOVIE_UPDATE_CHANNEL[0])] = legacy_msg_id

        for channel_id in MOVIE_UPDATE_CHANNEL:
            msg_id = message_ids.get(str(channel_id))
            if not msg_id:
                # If message doesn't exist for this channel, send it
                await send_movie_update(bot, base_name)
                # Re-fetch doc to get updated message_ids
                movie_doc = await db.movie_updates.find_one({"_id": base_name})
                message_ids = movie_doc.get("message_ids", {})
                # Skip editing since send_movie_update already sent the latest content
                continue
            ...
            try:
                ...
                await asyncio.sleep(e.value + 2)
                # Skip for now, will be updated next time
            except (MessageIdInvalid, MessageNotModified) as e:
                logger.warning(f"Message update skipped in {channel_id} due to error: {e}")
            except Exception as e:
                logger.error(f"Failed to update message in {channel_id}: {e}")
                try:
                    await bot.delete_messages(chat_id=channel_id, message_ids=msg_id)
