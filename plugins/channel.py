...
async def update_movie_message(bot, base_name):
    try:
        ...
        for chat_id_str, msg_id in message_ids.items():
            try:
                if is_photo:
                ...
                logger.warning(f"Message update skipped in {chat_id_str} due to error: {e}")
                continue
            except FloodWait as e:
                await asyncio.sleep(e.value + 2)
            except Exception:
                try:
                    ...
                    logger.error(f"Error during message deletion in recovery for {chat_id_str}: {e}")

                # If message is deleted or can't be updated, we might need to re-send to all
                # but let's just clear the ID for this channel for now
                new_message_ids = message_ids.copy()
                new_message_ids.pop(chat_id_str, None)
                if not new_message_ids:
                    await db.movie_updates.update_one(
                        {"_id": base_name},
                        {"$set": {"message_ids": {}, "is_photo": False}}
                    )
                    await send_movie_update(bot, base_name)
                else:
                    await db.movie_updates.update_one(
